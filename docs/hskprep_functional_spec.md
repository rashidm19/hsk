...
| hsk_checkout_started | localStorage | `{uid, ts}` — доказательство реально начатого чекаута, TTL 30 мин; взводится перед редиректом на эквайер, потребляется (удаляется) на возврате — без него ?pay=success не выдаёт грейс |
